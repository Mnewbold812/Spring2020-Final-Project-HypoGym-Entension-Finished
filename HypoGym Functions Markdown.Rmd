...
column, you need to specify when writing the inserted dataset, (and DailyUsePerYearMean requires that you do), usually by adding [,x] to the end, where x is the column you need to access. 
```{r}
...
assigned; the current grid has been chosen to establish maximum viewing comfort, but other adjustments might be neccesary based on user needs.
...
##Box Plot of Service Means by Year
```{r}
